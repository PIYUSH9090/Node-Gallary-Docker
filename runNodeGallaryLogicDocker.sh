...
abort()
{
    echo >&2 '
***************
*** ABORTED runNodeGallaryLogicDocker ***
***************
'
    echo "An error occurred runNodeGallaryLogicDocker . Exiting..." >&2
    exit 1
}
...
docker build -f Dockerfile -t $DOCKERUSER_ID/node-gallary-logic .
# Now image building is done 
# So now we will push that image to dockerhub
echo "Pushing node-gallary-logic"
docker push $DOCKERUSER_ID/node-gallary-logic
...
docker run -d -p 5050:5050 $DOCKERUSER_ID/node-gallary-logic &
sleep 5
echo "List of containers running now"
docker container ls -a

NodeGallaryLogicId="$(docker container ls -f ancestor="$DOCKERUSER_ID/node-gallary-logic" -f status=running -aq)"
echo " The one we just started is : $NodeGallaryLogicId"

if [ -n "$NodeGallaryLogicId" ]; then
  echo "node-gallary container is running $(docker container ls -f ancestor=$DOCKERUSER_ID/node-gallary-logic -f status=running -aq) :) "
else
...
echo >&2 '
************
*** DONE runNodeGallaryLogicDocker ***
************'